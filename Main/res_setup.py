...
def setup(seed, logfile):
    ...
    M10e = 0.00003002513*1 # Mass of ten Earths
    ...
    if numplanets == 5:
        ...
        a0 = (0.0, 13.2, 32.3, 67.2, 78.5, 96.3) ## Start further out to get into resonance
    elif numplanets == 4:
##        a0 = (0.0, 13.2, 32.3, 69.0, 91.0)
        a0 = (0.0, 13.2, 32.3, 75.0, 100.4)
    ...
    if resonance == True:
        startfile = open("StartFiles/start_seed{0}.txt".format(seed), "r")
...
def mergeParticles(reb_sim, collision):
    ...
    if id1 == 9 or id2 == 9:
        ...
        minpericount += 1
    logfile = open("LogFiles/log_seed{0}.txt".format(seed), "a")
...
def ejectParticle(sim, time, seed, logfile):    
    ...
    posfile.write("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\n".format(time, sim.particles[0].hash, sim.particles[0].x, sim.particles[0].y, sim.particles[0].z,
    ...
    for i in range(len(o)):           
        ...
        datafile.write("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\n".format(time, o[i].a, o[i].e,
                                                                              ...
                                                                              Eerror, sim.particles[i+1].hash))
        posfile.write("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\n".format(time, sim.particles[i+1].hash, sim.particles[i+1].x, sim.particles[i+1].y, sim.particles[i+1].z,
                                                                             sim.particles[i+1].vx, sim.particles[i+1].vy, sim.particles[i+1].vz, sim.particles[i+1].m))
    sim.remove(hash=mid)
    sim.move_to_com()
...
def hltau(seed):
    ...
    dataName = "DataFiles/data_seed{0}.txt".format(seed)
    logName = "LogFiles/log_seed{0}.txt".format(seed)
    checkPoint = "SaveStates/HL_Tau_Seed_{0}.bin".format(seed)
    checkPrev = "SaveStates/HL_Tau_Seed_{0}_prev.bin".format(seed)
    posPath = "PosFiles/pos_seed{0}.txt".format(seed)
    ...
    if (os.path.exists(checkPrev) == True):
        ...
        if times[tindex] < tcutoff:
            rebxtra = rebx.Extras(sim)
            params = rebxtra.add("modify_orbits_forces")
##            params2 = rebxtra.add("modify_mass")

            o = sim.calculate_orbits(heliocentric = True)
            for part in sim.particles[1:]:
                try:
                    if part.hash == 5:
                        part.params["tau_a"] = -taua0 / (o[-3].a - 64.2)
                    part.params["tau_e"] = -5.0e5# / (o[-3].a - 64.2)
##                    part.tau_mass = 8.5e6# / (o[-3].a - 64.2) 
                except:
                    pass
...
##            print "a"
            print time, o[-1].a, o[-2].a, o[-1].P/o[-2].P, o[-3].a, o[-2].P/o[-3].P, sim.particles[-1].m / Mj, sim.particles[-1].params["tau_a"]
##            resfile = open("ResFiles/res_seed{0}.txt".format(seed), "a")
##            resfile.write("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\n".format(time, o[-1].a, o[-2].a, o[-1].P/o[-2].P, o[-3].a, o[-2].P/o[-3].P, o[-1].e, o[-2].e, o[-3].e))
##            resfile.close()
        except:
            pass
        ...
        ## Save the simulation
        sim.save(checkPoint)
        system("mv SaveStates/HL_Tau_Seed_{0}.bin SaveStates/HL_Tau_Seed_{0}_prev.bin".format(seed))
