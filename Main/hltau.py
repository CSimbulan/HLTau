...
def ejectParticle(sim, time, seed, logfile, posfile, datafile):    
...
def hltau(seed):
    ...
    ## Set tau_mass0 depending on case
    if numplanets == 4:
        tau_m0 = 6.0e5
    elif numplanets == 5:
        tau_m0 = 8.5e5
    ...
    if (os.path.exists(checkPrev) == True):
        ...
        if times[tindex] < tcutoff:
            ...
            for part in sim.particles[1:]:
                try:
                    part.tau_a = np.inf
                    part.tau_e = -1.0e6 * np.exp(times[tindex2] / 3.0e6)
                    part.tau_mass = tau_m0 * np.exp(times[tindex2] / 3.0e6) 
                except:
                    pass
        ...
        for part in sim.particles[1:]:
            try:
                part.tau_a = np.inf
                part.tau_e = -1.0e6 * np.exp(1.0 / 3.0e6)
                part.tau_mass = tau_m0 * np.exp(1.0 / 3.0e6) 
            except:
                pass
        tindex = 0
        rebx_switch = True
    ...
    for i,time in enumerate(times[tindex:]):
        ...
        ## If a particle reaches past the max distance, treat is as ejected
            ...
            ejectParticle(sim, time, seed, logfile, posfile, datafile)
        ...
        if time < tcutoff and rebx_switch == True:
            ...
            for part in sim.particles[1:]:
                try:
                    part.tau_a = np.inf
                    part.tau_e = -1.0e6 * np.exp(time / 3.0e6)
                    part.tau_mass = tau_m0 * np.exp(time / 3.0e6) 
                except:
                    pass
        #try:
